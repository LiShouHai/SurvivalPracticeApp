...
onLoad(async (options) => {
  categoryId.value = options.categoryId ? Number.parseInt(options.categoryId, 10) : 4
  await fetchData()
})
...
<template>
  <view class="leg-page" :class="{ 'leg-page--locked': showAddModal || showDeleteModal }">
    <view class="bg-grid" />
    ...
    <view class="hero">
      <view class="hero-text">
        ...
        <text class="hero-title">腿部</text>
        <text class="hero-subtitle">股四头肌 · 腘绳肌 · 臀部 · 小腿</text>
      </view>
...
.leg-page {
  ...
  min-height: 100vh;
  background: linear-gradient(160deg, #0e1120 0%, #0a0c16 60%, #0b0f1a 100%);
...
.primary-btn {
  ...
  box-shadow: 0 18rpx 40rpx rgba(139, 156, 255, 0.3);
  transition:
...
.primary-btn:active {
  ...
  box-shadow: 0 12rpx 26rpx rgba(139, 156, 255, 0.2);
}
...
.section-count {
  ...
  border: 1rpx solid rgba(139, 156, 255, 0.35);
  display: flex;
  align-items: center;
...
.exercise-chip--selected {
  ...
  box-shadow: 0 4rpx 16rpx rgba(139, 156, 255, 0.1);
...
.modal {
  width: 100%;
  max-width: 620rpx;
  background: linear-gradient(150deg, rgba(23, 26, 45, 0.98), rgba(14, 16, 28, 0.96));
  border-radius: 28rpx;
...
.modal-glow {
  ...
  top: -90rpx;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(139, 156, 255, 0.35), transparent 70%);
  filter: blur(6rpx);
  opacity: 0.9;
  pointer-events: none;
  z-index: 0;
...
.modal-corner--tl {
  ...
  border-left-color: rgba(139, 156, 255, 0.75);
...
.modal-corner--br {
  ...
  border-right-color: rgba(139, 156, 255, 0.6);
...
.modal-title::after {
  ...
  height: 4rpx;
  border-radius: 999rpx;
  background: linear-gradient(90deg, rgba(139, 156, 255, 0.9), rgba(167, 139, 250, 0.8));
}
...
.group-chip--active {
  border-color: rgba(139, 156, 255, 0.6);
...
.modal-input:focus {
  ...
  box-shadow: 0 0 0 2rpx rgba(139, 156, 255, 0.15);
}
