...
onLoad(async (options) => {
  categoryId.value = options.categoryId ? Number.parseInt(options.categoryId, 10) : 3
  await fetchData()
})
...
const tips = ref([
  '推举动作避免锁死肘关节，保持肌肉持续张力。',
  '侧平举时注意小臂不要高于手肘，避免斜方肌过度代偿。',
])
...
<template>
  <view class="shoulder-page" :class="{ 'shoulder-page--locked': showAddModal || showDeleteModal }">
    <view class="bg-grid" />
    ...
    <view class="hero">
      <view class="hero-text">
        ...
        <text class="hero-title">肩部</text>
        <text class="hero-subtitle">前束 · 中束 · 后束 · 综合</text>
      </view>
...
.shoulder-page {
  ...
  min-height: 100vh;
  background: linear-gradient(160deg, #13110c 0%, #0f0d0a 60%, #0b0f1a 100%);
...
.bg-glow--1 {
  width: 520rpx;
  height: 520rpx;
  background: rgba(246, 180, 77, 0.15);
  top: -220rpx;
  right: -180rpx;
}
...
.primary-btn {
  ...
  box-shadow: 0 18rpx 40rpx rgba(246, 180, 77, 0.3);
  transition:
...
.primary-btn:active {
  ...
  box-shadow: 0 12rpx 26rpx rgba(246, 180, 77, 0.2);
}
...
.section-count {
  ...
  height: 44rpx;
  border-radius: 22rpx;
  background: rgba(246, 180, 77, 0.16);
  border: 1rpx solid rgba(246, 180, 77, 0.35);
  display: flex;
  align-items: center;
...
.exercise-chip--selected {
  background: rgba(246, 180, 77, 0.15);
  border-color: rgba(246, 180, 77, 0.5);
  box-shadow: 0 4rpx 16rpx rgba(246, 180, 77, 0.1);
...
.modal-glow {
  ...
  top: -90rpx;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(246, 180, 77, 0.35), transparent 70%);
  filter: blur(6rpx);
  opacity: 0.9;
  pointer-events: none;
  z-index: 0;
...
.modal-corner--tl {
  top: 10rpx;
  left: 10rpx;
  border-top-color: rgba(246, 180, 77, 0.75);
  border-left-color: rgba(246, 180, 77, 0.75);
...
.modal-corner--tr {
  ...
  border-right-color: rgba(255, 107, 107, 0.75);
...
.modal-corner--br {
  bottom: 10rpx;
  right: 10rpx;
  border-bottom-color: rgba(246, 180, 77, 0.6);
  border-right-color: rgba(246, 180, 77, 0.6);
...
.modal-title::after {
  ...
  height: 4rpx;
  border-radius: 999rpx;
  background: linear-gradient(90deg, rgba(246, 180, 77, 0.9), rgba(255, 107, 107, 0.8));
}
...
.group-chip--active {
  border-color: rgba(246, 180, 77, 0.6);
...
.modal-input:focus {
  border-color: rgba(246, 180, 77, 0.7);
  box-shadow: 0 0 0 2rpx rgba(246, 180, 77, 0.15);
}
...
.ghost-btn--active {
  ...
  border-color: rgba(246, 180, 77, 0.4);
}
