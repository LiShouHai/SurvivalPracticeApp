...
onLoad(async (options) => {
  categoryId.value = options.categoryId ? Number.parseInt(options.categoryId, 10) : 1
  await fetchData()
})
  ...
  if (itemToDelete.value) {
    try {
      ...
      if (!Number.isFinite(deleteId) || deleteId <= 0) {
        uni.showToast({ title: '动作ID无效，无法删除', icon: 'none' })
        showDeleteModal.value = false
        itemToDelete.value = null
        return
      }
      // 如果是系统动作（id 是数字），后端逻辑可能不允许删除，这里假设只删自定义的
...
<template>
  <view class="chest-page" :class="{ 'chest-page--locked': showAddModal || showDeleteModal }">
    <view class="bg-grid" />
    ...
    <view class="hero">
      <view class="hero-text">
        ...
        <text class="hero-title">胸部</text>
        <text class="hero-subtitle">上胸 · 中胸 · 下胸 · 辅助</text>
      </view>
...
.chest-page {
  ...
  --accent: #29e3b1;
  --accent-strong: #0dd6a7;

  min-height: 100vh;
  background: linear-gradient(160deg, #0c1324 0%, #0a0f1b 60%, #0b0f1a 100%);
  padding: 88rpx 24rpx calc(180rpx + env(safe-area-inset-bottom));
  color: var(--text);
  position: relative;
  overflow: hidden;
}
...
.bg-glow--1 {
  width: 520rpx;
  height: 520rpx;
  background: rgba(41, 227, 177, 0.18);
  top: -220rpx;
  right: -180rpx;
}
...
.bg-glow--2 {
  width: 420rpx;
  height: 420rpx;
  background: rgba(104, 216, 255, 0.16);
  bottom: 60rpx;
  left: -200rpx;
}
...
.primary-btn {
  ...
  box-shadow: 0 18rpx 40rpx rgba(41, 227, 177, 0.35);
  transition:
...
.primary-btn:active {
  ...
  box-shadow: 0 12rpx 26rpx rgba(41, 227, 177, 0.28);
}
...
.section-count {
  ...
  height: 44rpx;
  border-radius: 22rpx;
  background: rgba(41, 227, 177, 0.16);
  border: 1rpx solid rgba(41, 227, 177, 0.35);
  display: flex;
  align-items: center;
...
.exercise-chip--selected {
  background: rgba(41, 227, 177, 0.15);
  border-color: rgba(41, 227, 177, 0.5);
  box-shadow: 0 4rpx 16rpx rgba(41, 227, 177, 0.1);
...
.modal-glow {
  ...
  top: -90rpx;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(41, 227, 177, 0.35), transparent 70%);
  filter: blur(6rpx);
  opacity: 0.9;
  pointer-events: none;
  z-index: 0;
...
.modal-corner--tl {
  top: 10rpx;
  left: 10rpx;
  border-top-color: rgba(41, 227, 177, 0.75);
  border-left-color: rgba(41, 227, 177, 0.75);
...
.modal-corner--tr {
  top: 10rpx;
  right: 10rpx;
  border-top-color: rgba(104, 216, 255, 0.75);
  border-right-color: rgba(104, 216, 255, 0.75);
...
.modal-corner--bl {
  bottom: 10rpx;
  left: 10rpx;
  border-bottom-color: rgba(104, 216, 255, 0.6);
  border-left-color: rgba(104, 216, 255, 0.6);
...
.modal-corner--br {
  bottom: 10rpx;
  right: 10rpx;
  border-bottom-color: rgba(41, 227, 177, 0.6);
  border-right-color: rgba(41, 227, 177, 0.6);
...
.modal-title::after {
  ...
  height: 4rpx;
  border-radius: 999rpx;
  background: linear-gradient(90deg, rgba(41, 227, 177, 0.9), rgba(104, 216, 255, 0.8));
}
...
.group-chip--active {
  border-color: rgba(41, 227, 177, 0.6);
  background: rgba(41, 227, 177, 0.18);
}
...
.modal-input:focus {
  border-color: rgba(41, 227, 177, 0.7);
  box-shadow: 0 0 0 2rpx rgba(41, 227, 177, 0.15);
}
...
.ghost-btn--active {
  background: rgba(41, 227, 177, 0.12);
  border-color: rgba(41, 227, 177, 0.4);
}

.ghost-btn--active .ghost-btn-text {
  color: var(--accent);
}
